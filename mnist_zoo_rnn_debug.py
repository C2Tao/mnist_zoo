...
def seq_lstm(x, n_output):
    ...
    with tf.variable_scope('lstm') as vs:
        ...
        print map(lambda g: g.name, lstm_variables)
        print map(lambda g: g.get_shape(), lstm_variables)
        ...
        #new_w = w.assign(np.zeros(lstm_variables[0].get_shape[0]))
        '''
        w = lstm_variables[0] 
        w_np = np.zeros([w.get_shape()[0], w.get_shape()[1]])
        w.assign(w_np)

        b = lstm_variables[1]
        b_np = np.zeros([b.get_shape()[0]])
        b.assign(b_np)
        '''
        #sess.run(tf.initialize_all_variables())
        #print sess.run(b)
        
        # get lstm internal variables [weight, bias]
